...
export default function AdminDashboard() {
  ...
  const retryTranslations = async () => {
    if (!retryStats || retryStats.total === 0) {
      setRetryMessage('No videos need translation retry')
      return
    }

    setRetryLoading(true)
    setRetryMessage('Starting translation retry...')
    ...
    try {
      ...
      if (data.success) {
        setRetryMessage(`✅ ${data.successful} translations succeeded, ${data.failed} failed`)
        await fetchRetryStats()
        await fetchStats()
      } else {
        setRetryMessage(`❌ Failed: ${data.message}`)
      }
    } catch (error) {
      setRetryMessage(`❌ Error: ${error instanceof Error ? error.message : 'Unknown error'}`)
    } finally {
    ...
    <div className="px-4 sm:px-0 space-y-6">
      ...
      <div className="bg-card border border-border rounded-lg p-6">
        ...
        <p className="text-muted-foreground mb-4">
          Retry translations for videos that failed during scraping due to rate limits.
        </p>
        ...
        {retryStats && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div className="bg-muted/50 border border-border rounded-lg p-4">
              <div className="text-sm text-muted-foreground mb-1">Videos Needing Translation</div>
              <div className="text-3xl font-bold text-primary">{retryStats.total}</div>
            </div>

            {retryStats.byRetryCount.length > 0 && (
              <div className="bg-muted/50 border border-border rounded-lg p-4">
                <div className="text-sm text-muted-foreground mb-2">Retry Attempts</div>
                <div className="space-y-1">
                  {retryStats.byRetryCount.map(item => (
                    <div key={item.retries} className="text-sm flex justify-between">
                      <span className="text-foreground">{item.retries} attempts:</span>
                      <span className="font-medium text-primary">{item.count} videos</span>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <div className="bg-muted/50 border border-border rounded-lg p-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-3">
            <div>
              <label className="block text-sm font-medium text-foreground mb-2">
                Retry Limit (max videos per batch)
              </label>
              <input
                type="number"
                min="10"
          ...
          <div className="flex gap-3 items-center">
            <button
              ...
              disabled={retryLoading || !retryStats || retryStats.total === 0}
            ...
            >
              {retryLoading ? 'Retrying...' : `Retry Failed Translations (${retryStats?.total || 0})`}
            </button>
